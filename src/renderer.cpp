	...
	**/
	for (int i = 0; i < nrow; i++) {
		for (int j = 0; j < ncol; j++) {
			...
			reset_emitted_ray_counter_();
		}
	}
}

Color Renderer::trace_ray_(const Ray &ray)
{
	increment_emitted_ray_counter_();
	Point hit_location;
	...
	if (is_hit_(obj)) {
		...
		if (emitted_ray_counter_ < max_rays_) {
			...
			color += trace_ray_(reflected_ray)*obj->reflection_intensity();
		}
	}
	return color;
}

bool Renderer::is_hit_(const Sphere* obj) const
{
	return obj;
}

void Renderer::color_at_(const Sphere* obj, const Point& hit_location, const Vector3d<double>& hit_normal_dir, Color& color) const 
{
	// ambient color
...
void Renderer::set_pixel_color_(int irow, int jcol, Color pixel_color)
{
	scene_->image()->set_pixel(irow, jcol, pixel_color);
}

void Renderer::increment_emitted_ray_counter_()
{
	emitted_ray_counter_++;
}

void Renderer::reset_emitted_ray_counter_()
{
	emitted_ray_counter_ = 0;
}

/**
	One possible way of parallelizing
	1. Make grid and blocks such that 1 thread handles 1 pixel
	2. No need to copy pixel's Color into shared memory
	   since 
