	...
	**/
	...
	for (int i = 0; i < nrow; i++) {
		for (int j = 0; j < ncol; j++) {
			...
			Vector3d<float> dir = scene_->camera_location().direction_to(pixel_loc);
...
CUDA_CALLABLE void Renderer::color_at_(const Sphere* obj, const Point& hit_location, const Vector3d<float>& hit_normal_dir, Color& color) const 
{
	...
	for (int i = 0; i < scene_->n_lights(); i++) {
		...
		color += obj->base_color()*std::fmax(hit_to_light.dot(hit_normal_dir), 0.0f)*obj->diffuse_coefficient();
		
		// specular color - Phong shading model
		Point R = hit_to_light + hit_to_cam;
		R.normalize();
		color += current_light->color()*obj->specular_coefficient()*pow(std::fmax(hit_normal_dir.dot(R), 0.0f), 50.f);
	}
}
